...
def document_type(ptype: typing.Type, description: typing.Optional[str] = None) -> typing.Dict[str, typing.Any]:
    origin = typing.get_origin(ptype)
    args = typing.get_args(ptype)
    ...
    if is_optional(ptype):
        ...
        out = {
            "type": "object",
            "patternProperties": {
                "^[a-zA-Z0-9_]+$": document_type(args[1])
            }
        }
    elif origin == list:
        out = {
            "type": "array",
            "items": document_type(args[0])
        }
    elif ptype in ATOM_TYPES:
        out = {
            "type": ATOM_TYPES[ptype]
        }
        ...
        out = {
            "type": "string",
            "enum": [e.value for e in ptype]
        }
    else:
        properties = {}
        required = []
        for field in dataclasses.fields(ptype):
            if not is_optional(field.type) and isinstance(field.default, dataclasses._MISSING_TYPE) and isinstance(field.default_factory, dataclasses._MISSING_TYPE):
                required.append(field.name)

            properties[field.name] = document_type(field.type, field.metadata.get("comment"))
